...
let private expandError (code: int) (needle: string) (source: string) =
...
[<Fact>]
let ``binder expand: identity macro`` () =
    let exprs =
        expandOk
            """
(define-syntax id
...
[<Fact>]
let ``binder expand: constant pattern match`` () =
    let exprs =
        expandOk
            """
(define-syntax foo
...
[<Fact>]
let ``binder expand: no-arg constant pattern (foo)`` () =
    let exprs =
        expandOk
            """
(define-syntax foo
...
[<Fact>]
let ``binder expand: ellipsis macro`` () =
...
[<Fact>]
let ``binder expand: let-syntax scoping`` () =
...
[<Fact>]
let ``binder expand: hygienic rename`` () =
...
[<Fact>]
let ``binder expand: recursive macro via keyword-head binding`` () =
...
[<Fact>]
let ``binder expand: custom ellipsis macro`` () =
...
[<Fact>]
let ``binder expand: dotted-tail pattern (underscore) matches proper list tail`` () =
...
[<Fact>]
let ``binder expand: dotted-tail pattern selects over fixed-arity rules`` () =
...
[<Fact>]
let ``binder expand: define in let body produces local not global`` () =
...
[<Fact>]
let ``binder expand: define at true top level produces global`` () =
...
[<Fact>]
let ``binder expand: nested let scopes do not leak defines as globals`` () =
...
[<Fact>]
let ``binder expand: define in let-syntax body does not shadow outer binding`` () =
...
[<Fact>]
let ``binder expand: literal keyword not matched when shadowed by local variable`` () =
